...
<div class="modal fade" id="modalEditPenempatan-{{ $row->id }}" tabindex="-1">
 <div class="modal-dialog modal-dialog-centered modal-lg">
  <form action="{{ route('admin.penempatan.update',$row->id) }}" method="POST" class="modal-content">
   ...
   <div class="modal-header">
    <h5 class="modal-title">Edit Penempatan {{ $row->pengajuan->nama_siswa }}</h5>
    ...
    <div class="row g-3">
      ...
      <div class="col-12 col-md-6">
        ...
        <select name="bidang_id" class="form-select" required>
            @foreach($bidang as $b)
              <option value="{{ $b->id }}" {{ $b->id==$row->bidang_id?'selected':'' }}>
                {{ $b->nama }}
              </option>
            @endforeach
        </select>
   ...
   <div class="modal-footer">
    ...
    <button type="submit" class="btn btn-outline-primary ms-auto">Update</button>
   </div>
  </form>
 </div>
</div>
