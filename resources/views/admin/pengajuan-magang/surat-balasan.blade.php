...
<div class="no-surat">
    ...
    <p>
        Yth. Dekan Fakultas Teknik<br>
        {{ $pengajuan->universitas ?? '-' }}<br>
        di Tempat
    </p>
</div>
...
<div class="content">
    <p>
        Menindaklanjuti Surat Dekan Fakultas Teknik {{ $pengajuan->universitas ?? '-' }}
        Nomor: {{ $pengajuan->no_surat }} tanggal {{ \Carbon\Carbon::parse($pengajuan->tgl_surat)->translatedFormat('d F Y') }} hal Permohonan PKL, 
        dengan ini kami sampaikan bahwa usulan mahasiswa berikut:
    </p>

    <table class="table-data">
        <tr><td>Nama</td><td>: {{ $pengajuan->nama_mahasiswa }}</td></tr>
        <tr><td>Jurusan</td><td>: {{ $pengajuan->jurusan }}</td></tr>
        <tr><td>Email</td><td>: {{ $pengajuan->email_mahasiswa }}</td></tr>
    </table>
