...
<div class="container py-4">
    ...
    <h3 class="mb-4 text-center">Pengaturan Akun Magang</h3>

@if($statusProfile)
    <div class="alert alert-success d-flex align-items-center rounded-3">
        <i class="ti ti-check-circle me-2"></i>
        <div>Profile Magang sudah lengkap.</div>
    </div>
@else
    <div class="alert alert-warning d-flex align-items-center rounded-3">
        <i class="ti ti-alert-triangle me-2"></i>
        <div>Profile Magang belum lengkap. Lengkapi data & upload foto.</div>
    </div>
@endif
    ...
    <div class="card-header bg-primary text-white">
        <h5 class="mb-0">Form Profile Magang</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('magang.profile.update') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="universitas" class="form-control @error('universitas') is-invalid @enderror" 
                           value="{{ old('universitas', $profile->universitas) }}" required>
                    @error('universitas')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                @if($profile->foto)
                    <div class="mt-2 d-flex justify-content-center">
                        <img src="{{ asset('uploads/foto_siswa/'.$profile->foto) }}" 
                             alt="Foto Magang" class="rounded-circle border" style="width:120px; height:120px;">
                    </div>
                @endif
                @error('foto')
